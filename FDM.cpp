...
void FDMEulerExplicit::step_march() {
  ...
  std::ofstream finaly("finally.csv");
    std::ofstream fichier1("fichier1.csv");
  ...
  while(cur_t < t_dom) {
    ...
    for (int j=0; j<J; j++) {
       ...
       if (prev_t==((N-8)*dt))
         ...
         theorique<<x_values[j]<<" "<<new_result[j] << std::endl;
       else if (prev_t==0)
       {
           finaly<<x_values[j]<<" "<<new_result[j] << std::endl;
       }
       /*else if (prev_t==((8*dt)))
       {
           fichier1<<x_values[j]<<" "<<new_result[j] << std::endl;}*/
